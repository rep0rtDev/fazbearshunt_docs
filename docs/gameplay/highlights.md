...
Просвечивает `ply` для локального игрока на заданное время, с указанным цветом.

| Параметр | Тип | Описание | Зн. по умолчанию |
|---|---|---|---|
| `ply` | `Player` | Кто просвечен | |
| `duration` | `float` *(опц.)* | Длительность в секундах | `ConVar halo_time` |
...
Убирает просвет для локального игрока на `ply`.
